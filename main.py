...
def main(redis_client):
    ...
    for i in tList:
        ...
        try:
            sold = BeautifulSoup(str(originTag.find_all(class_="title mn-0-0-15")[0]), "html.parser").get_text()
        except:
        ...
        embed.set_footer(text = f'{sold}', icon_url = "https://i.imgur.com/LwvJYdH.png")

        if int(price.replace(" ", "").replace(",", "")) > 3000:
            pass
        else:
